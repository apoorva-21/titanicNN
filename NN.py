...
W_Out = np.random.rand(n_Out, n_HL1) / np.sqrt(n_HL1)
b_Out = np.zeros((n_Out, 1)) / np.sqrt(n_HL1)


def forward_prop(X_in):
	global W_HL1, b_HL1,W_Out, b_Out
	...
	z_Out = np.matmul(W_Out, a_1) + b_Out
	a_Out = sigmoid(z_Out)

	return a_1, a_Out

def backward_prop(X_in, Y_true, a_1, a_Out):

	dz_Out = a_Out - Y_true
	dW_Out = np.matmul(dz_Out, a_1.T)
	# print dW_Out
	# print dW_Out.shape
	...
	return dW_HL1, db_HL1, dW_Out, db_Out
...
for i in range(N_EPOCHS):
	...
	#get activations::
	a_1, a_Out = forward_prop(X)

	#get gradients::
	dW_HL1, db_HL1, dW_Out, db_Out = backward_prop(X, y, a_1, a_Out)
...
a_1_train, a_Out_train = forward_prop(X)
# print a_Out_test
# exit()
correct = 0
...
a_1_test, a_Out_test = forward_prop(X_test)
# print a_Out_test
# exit()
correct = 0
